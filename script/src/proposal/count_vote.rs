...
/// Mutable state accumulated while tallying votes over the voting block range.
///
/// It bundles the two bookkeeping maps and the running cycle counter so they can
/// be passed through `process_block` as a single argument.
struct TallyState {
    /// Voter lock-script hash -> (vote direction, vote amount).
    /// A later vote from the same voter overwrites the earlier one.
    vote_map: BTreeMap<[u8; 32], (u8, u64)>,
    /// DAO deposit out point -> voter lock-script hash.
    /// If any transaction spends an out point in this map, the associated voter
    /// is removed from `vote_map` to prevent double counting.
    dao_outpoint_to_voter: BTreeMap<[u8; 36], [u8; 32]>,
    /// Cycles accumulated by the tally so far, charged for each non-constant-time
    /// operation performed during the scan.
    cycles: Cycle,
}

impl TallyState {
    fn new(cycles: Cycle) -> Self {
        TallyState {
            vote_map: BTreeMap::new(),
            dao_outpoint_to_voter: BTreeMap::new(),
            cycles,
        }
    }
}
    ...
    *cycles += (block.data().total_size() as Cycle) * 4;
    ...
    for tx in block.transactions() {
        ...
        for j in 0..outputs.len() {
            let output = outputs.get(j).ok_or_else(unexpected)?;
...
pub fn count_vote<B: BlockProvider + ?Sized>(
    ...
    let mut state = TallyState::new(1_000_000);
    ...
    let minimal_req = u64::from_le_bytes(
        proposal_reader
            ...
            .as_slice()
            .try_into()
            .map_err(|_| ScriptError::validation_failure(proposal_script, ERROR_UNEXPECTED))?,
    );
    ...
    for (direction, amount) in state.vote_map.values() {
        if *direction == 1 {
    ...
    let min_shannon = minimal_req
        .checked_mul(100_000_000)
        .ok_or_else(|| ScriptError::validation_failure(proposal_script, ERROR_OVERFLOW))?;
