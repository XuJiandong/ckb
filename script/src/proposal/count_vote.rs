...
fn find_proposal(blocks: &[BlockView], proposal_script: &Script) -> Option<Proposal> {
    let first_block = blocks.first()?;
    for tx in first_block.transactions() {
        let data = tx.data();
        let raw = data.raw();
...
pub fn count_vote(blocks: &[BlockView], proposal_script: &Script) -> VoteResult {
